...
public class MemberMain {
	...
	public static void main(String[] args) throws IOException {
		...
		// 스프링에서는 xml파일 읽어올 때 ApplicationContext를 공장으로 사용 
		ApplicationContext factory
		= new ClassPathXmlApplicationContext("ch24/spring/mybatis/spring-mybatis.xml");
...
//		List<MemberDTO> list = session.selectList("memberlist"); // resultType은 memberDTO였음.  레코드 1개는 MemberDTO타입.(mybastis.MemberDTO의 별명)
		...
		*/
		...
		// MemberDAO dao = new MemberDAO(); // @Repository 로 대체 
		// dao.setSession(session); // @Autowired 로 대체 
		
//		MemberServiceImpl service = new MemberServiceImpl();
//		service.setDao(dao); // @Autowired 로 주입해줄 수 있다 
